...
export const Card = forwardRef<HTMLDivElement, CardProps>(({ customClass, ...rest }, ref) => (
  <div
    ref={ref}
    {...rest}
    className={`absolute top-1/2 left-1/2 rounded-xl border border-white/10 bg-[#0a0b10] [transform-style:preserve-3d] [will-change:transform] ${customClass ?? ''} ${rest.className ?? ''}`.trim()}
  />
));
...
const makeSlot = (i: number, dx: number, dy: number, total: number): Slot => ({ x: i*dx, y: -i*dy, z: -i*dx*1.5, zIndex: total-i });
...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  useEffect(() => {
    ...
    const swap = () => {
      ...
      rest.forEach((idx, i) => {
        const el = refs[idx].current!, slot = makeSlot(i, cardDistance, verticalDistance, refs.length);
    ...
    if (pauseOnHover) {
      ...
      const pause = () => { tlRef.current?.pause(); clearInterval(intervalId); };
      const resume = () => { tlRef.current?.play(); };
      node.addEventListener('mouseenter', pause);
      node.addEventListener('mouseleave', resume);
      return () => { node.removeEventListener('mouseenter', pause); node.removeEventListener('mouseleave', resume); clearInterval(intervalId); };
    }
  ...
  return (
    <div ref={container} className="relative" style={{ width, height, perspective: '900px' }}>
      {rendered}
    </div>
  );
};
