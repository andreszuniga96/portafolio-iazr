...
function LayerStack({
  ...
  React.useEffect(() => {
    if (!isMobile || containerWidth === 0) { setAutoHeight(stageHeight); return; }
  ...
  useMotionValueEvent(scrollX, 'change', (v) => setProgressValue(v * 100));

  const maxScroll = React.useMemo(
    () => Math.max(0, cards.length * (effectiveCardWidth + cardGap) - cardGap - effectiveCardWidth),
  ...
  React.useEffect(() => {
    ...
    let isTouching = false, isHorizontalScroll = false;

    const onTouchStart = (e: TouchEvent) => {
      if (rafRef.current) { cancelAnimationFrame(rafRef.current); rafRef.current = null; }
      isTouching = true; isHorizontalScroll = false;
      startX = e.touches[0].clientX; startY = e.touches[0].clientY;
      startScroll = rawScroll.current;
      lastTouchX.current = startX; lastTouchTime.current = Date.now(); velocityRef.current = 0;
    };
    const onTouchMove = (e: TouchEvent) => {
      if (!isTouching) return;
      const cx = e.touches[0].clientX, cy = e.touches[0].clientY;
      const dx = Math.abs(cx - startX), dy = Math.abs(cy - startY);
      if (!isHorizontalScroll && dx < 10 && dy < 10) return;
      if (!isHorizontalScroll) isHorizontalScroll = dx > dy;
      if (isHorizontalScroll) {
        e.preventDefault();
        const now = Date.now(), dt = now - lastTouchTime.current;
        if (dt > 0) velocityRef.current = (lastTouchX.current - cx) / dt;
        lastTouchX.current = cx; lastTouchTime.current = now;
        if (rafRef.current) cancelAnimationFrame(rafRef.current);
        rafRef.current = requestAnimationFrame(() => {
          updateScrollX(startScroll + (startX - cx) * mobileSensitivity);
          rafRef.current = null;
        });
      }
    };
    const onTouchEnd = () => {
      if (!isHorizontalScroll) { isTouching = false; return; }
      isTouching = false;
      const velocity = velocityRef.current * 250 * mobileSensitivity;
      if (Math.abs(velocity) > 15) {
        const target = Math.max(0, Math.min(maxScroll, rawScroll.current + velocity * 0.4));
        animate(rawScroll.current, target, {
          type: 'spring', stiffness: 100, damping: 18, mass: 0.3,
  ...
  React.useEffect(() => {
    ...
    const onMouseDown = (e: MouseEvent) => {
      ...
      isDragging = true; dragStartX = e.clientX; dragStartScroll = rawScroll.current;
      e.preventDefault();
    };
    const onMouseMove = (e: MouseEvent) => { if (!isDragging) return; updateScrollX(dragStartScroll + (dragStartX - e.clientX)); };
  ...
  return (
    <div className={cn('w-full select-none', className)}>
      <div
        ...
        style={{ height: autoHeight, perspective: isMobile ? 1000 : 1400, transition: 'height 0.2s ease' }}
      >
        {cards.map((card, i) => {
          const isLastCard = i === cards.length - 1;
          const renderedWidth = lastCardFullWidth && isLastCard && containerWidth > 0 ? containerWidth : effectiveCardWidth;
          return (
            <StackCardInternal
              key={i}
              index={i}
      ...
      <div className="mt-4 flex flex-col items-center gap-2">
        ...
        />
        <p className="text-[10px] font-medium tracking-[0.14em] uppercase text-muted-foreground font-poppins">
          {isMobile ? 'desliza' : 'arrastra o desplaza'}
        </p>
      </div>
    </div>
  );
